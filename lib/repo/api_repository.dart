...
import '../features/getQuote/model/get_download_quote_data_model.dart';
        ...
        if (jsonMap["Response"]["Status"].toLowerCase() == "success") {
          ...
          switch (endpoint) {
            ...
            case APIUrls.getRemoveFromCart:
              ...
              return Success(code: APICode.SUCCESS, response: responseData);
            // =====
            case APIUrls.getCartCount:
              if (outerResponse.data is List) {
                responseData = (outerResponse.data as List<dynamic>)
                    .map(
                      (e) => GetCartCountDataModel.fromJson(
                          e as Map<String, dynamic>),
                    )
                    .toList();
              } else if (outerResponse.data is Map) {
                responseData = GetCartCountDataModel.fromJson(
                    outerResponse.data as Map<String, dynamic>);
              }
              return Success(code: APICode.SUCCESS, response: responseData);
            // =====
            case APIUrls.getStyles:
              if (outerResponse.data is List) {
                responseData = (outerResponse.data as List<dynamic>)
                    .map(
                      (e) => GetStylesDataModel.fromJson(
                          e as Map<String, dynamic>),
                    )
                    .toList();
              } else if (outerResponse.data is Map) {
                responseData = GetStylesDataModel.fromJson(
                    outerResponse.data as Map<String, dynamic>);
              }
              return Success(code: APICode.SUCCESS, response: responseData);
            // =====
            case APIUrls.getTnc:
              if (outerResponse.data is List) {
                responseData = (outerResponse.data as List<dynamic>)
                    .map(
                      (e) =>
                          GetTNCDataModel.fromJson(e as Map<String, dynamic>),
                    )
                    .toList();
              } else if (outerResponse.data is Map) {
            ...
            case APIUrls.insertQuoteData:
              ...
              return Success(code: APICode.SUCCESS, response: responseData);
            // =====
            case APIUrls.getQuote:
              if (outerResponse.data is List) {
                responseData = (outerResponse.data as List<dynamic>)
                    .map(
                      (e) => GetDownloadQuoteDataModel.fromJson(
                          e as Map<String, dynamic>),
                    )
                    .toList();
              } else if (outerResponse.data is Map) {
